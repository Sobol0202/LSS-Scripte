...
// Es handelt sich hier um eine nicht funktionelle Dev-Version. Sie funktioniert nicht und wird dies auch nie tun. Der Versuch sie zu reparieren sollte gar nicht erst unternommen werden.
...
(function() {
    ...
    function updateModal(modal, vehicleData, buildingData) {
        ...
        /*
        const assignAllButton = document.createElement('button');
        assignAllButton.textContent = 'Alle zuweisen';
        assignAllButton.style.backgroundColor = '#27ae60';
        assignAllButton.style.color = 'white';
        assignAllButton.style.border = 'none';
        assignAllButton.style.padding = '10px 20px';
        assignAllButton.style.marginBottom = '20px';
        assignAllButton.style.cursor = 'pointer';
        assignAllButton.style.borderRadius = '5px';
        modal.append(assignAllButton);

        assignAllButton.addEventListener('click', async () => {
            assignAllButton.disabled = true;

            const visibleVehicleIds = Array.from(table.querySelectorAll('tr'))
            .slice(1) // Header auslassen
            ...
            const total = visibleVehicleIds.length;
            let current = 0;

            for (const vehicleId of visibleVehicleIds) {
                current++;
                assignAllButton.textContent = `Zuweisung (${current} / ${total})`;
                await assignVehicle(vehicleId);
                await new Promise(res => setTimeout(res, 200));
            }

            assignAllButton.textContent = 'Zuweisung abgeschlossen!';
        });
        ...
        async function assignVehicle(vehicleId) {
            return new Promise(resolve => {
                ...
                iframe.src = `https://www.leitstellenspiel.de/vehicles/${vehicleId}/zuweisung`;
                ...
                iframe.onload = () => {
                    try {
                        ...
                        const observer = new MutationObserver(() => {
                            const okButton = doc.querySelector('#vehicle-assigner-button-group .btn-success');
                            if (okButton) {
                                okButton.click();

                                const checkInterval = setInterval(() => {
                                    const maxEl = doc.querySelector('#navbar-personal-collapse .label.label-success:nth-child(2)');
                                    const assignedEl = doc.querySelector('#count_personal');

                                    if (!maxEl || !assignedEl) return;

                                    const max = parseInt(maxEl.textContent.trim(), 10);
                                    const assigned = parseInt(assignedEl.textContent.trim(), 10);

                                    if (!isNaN(max) && !isNaN(assigned) && assigned >= max) {
                        ...
                        const okButtonAlready = doc.querySelector('#vehicle-assigner-button-group .btn-success');
                        if (okButtonAlready) {
                            okButtonAlready.click();
                        ...
                        console.error(`Fehler bei Fahrzeug ${vehicleId}`, e);
                        iframe.remove();
                        resolve();
                    }
                };
