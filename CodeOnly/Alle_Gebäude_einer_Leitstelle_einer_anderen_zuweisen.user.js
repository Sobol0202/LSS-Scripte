...
function createDropdowns(leitstellen) {
    ...
    label1.textContent = 'Quell Leitstelle:';
    const select1 = document.createElement('select');

    const label2 = document.createElement('label');
    label2.textContent = 'Ziel Leitstelle:';
    ...
    button.onclick = () => {
        ...
        if (selectedId1 && selectedId2 && selectedId1 !== selectedId2) {
            ...
            alert("Bitte wählen Sie zwei unterschiedliche Leitstellen aus.");
        }
    };
...
async function updateBuildings(sourceLeitstelleId, targetLeitstelleId) {
    try {
        ...
        // Sicherheitsabfrage
        if (!confirm(`${buildingsToUpdate.length} Gebäude werden einer neuen Leitstelle zugewiesen. Fortfahren?`)) {
            return;
        }
